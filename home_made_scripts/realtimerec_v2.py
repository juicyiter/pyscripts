...
def update_plot(frame):
    ...
    """
    global plotdata, count, freq, fft
    count += 1
    ...
    for i in range(len(args.channels)):
        tempfft = np.fft.fft(tempdata[:, i].reshape(len(tempdata[:, i]),))
        ...
        if i == 0:
            ...
            l = len(tempfft) - fft.shape[0]
            if l > 0:
                fft = np.append(np.zeros(l-1, fft.shape[1]), fft, axis=0)
                freq = np.append(np.zeros(l-1, fft.shape[1]), freq, axis=0)
            elif l < 0:
                tempfft = np.append(np.zeros(-l-1,), tempfft, axis=0)
                tempfreq = np.append(np.zeros(-l-1,), tempfreq, axis=0)
